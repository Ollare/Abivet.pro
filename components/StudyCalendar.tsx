    ...
    <div className="bg-white rounded-[3rem] shadow-sm border border-gray-100 overflow-hidden flex flex-col">
      <div className="p-8 border-b border-gray-100 flex items-center justify-between bg-emerald-50/30">
        ...
        <div className="flex gap-2">
          <button onClick={handlePrevMonth} className="w-10 h-10 flex items-center justify-center bg-white hover:bg-emerald-100 rounded-xl transition-colors border border-gray-200 shadow-sm">&larr;</button>
          <button onClick={handleNextMonth} className="w-10 h-10 flex items-center justify-center bg-white hover:bg-emerald-100 rounded-xl transition-colors border border-gray-200 shadow-sm">&rarr;</button>
        </div>
      </div>
        ...
        {Array.from({ length: daysInMonth }).map((_, i) => {
          ...
          return (
            <div 
              ...
              className={`min-h-[80px] md:min-h-[120px] p-2 rounded-2xl border transition-all cursor-pointer flex flex-col items-center justify-start relative overflow-hidden ${isSelected ? 'border-emerald-500 bg-emerald-50 ring-2 ring-emerald-100' : 'border-gray-50 hover:border-emerald-200 hover:bg-gray-50'}`}
            >
              <span className={`text-[10px] md:text-sm font-black mb-1 ${isToday ? 'bg-emerald-600 text-white w-6 h-6 md:w-8 md:h-8 flex items-center justify-center rounded-xl' : 'text-gray-400'}`}>
                {day}
              </span>
              ...
              {dayAccuracy !== null && (
                <div className={`mt-auto text-[8px] md:text-[10px] font-black px-2 py-0.5 rounded-full ${dayAccuracy >= 60 ? 'text-emerald-600 bg-emerald-100/50' : 'text-rose-500 bg-rose-100/50'}`}>
                  {dayAccuracy}%
                </div>
              )}
            </div>
        ...
        <div className="p-8 md:p-12 bg-gray-50 border-t border-gray-100 animate-fadeIn space-y-8">
          <div className="flex flex-col md:flex-row gap-6 items-end">
            <div className="flex-1 space-y-3 w-full">
              ...
              <select value={selectedSubject} onChange={(e) => setSelectedSubject(e.target.value)} className="w-full p-4 rounded-2xl border border-gray-200 text-xs font-bold focus:ring-4 focus:ring-emerald-100 outline-none bg-white">
                {allSubjects.map(s => <option key={s} value={s}>{s}</option>)}
              </select>
            </div>
            <button onClick={() => onAddReminder({ date: selectedDay, subject: selectedSubject, completed: false })} className="w-full md:w-auto bg-emerald-600 text-white px-10 py-4 rounded-2xl font-black text-xs shadow-xl active:scale-95 transition-all uppercase tracking-widest">Pianifica 🦴</button>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
               {reminders.filter(r => r.date === selectedDay).map(r => (
                 <div key={r.id} className="bg-white p-5 rounded-2xl border border-gray-100 flex items-center justify-between shadow-sm group">
                   <div className="flex items-center gap-4">
                     <input type="checkbox" checked={r.completed} onChange={() => onToggleReminder(r.id)} className="w-5 h-5 text-emerald-600 rounded-lg" />
               ...
               {badges.filter(b => new Date(b.earnedDate).toISOString().split('T')[0] === selectedDay).map(b => (
                 <div key={b.id} className="bg-emerald-50 p-5 rounded-2xl border border-emerald-100 flex items-center gap-4 shadow-sm">
                   <span className="text-3xl">🏆</span>
                   <div className="flex flex-col">
                     <span className="text-[10px] font-black text-emerald-600 uppercase tracking-widest leading-none mb-1">Badge Conquistato</span>
                     <span className="text-sm font-black text-emerald-900">{b.subject}</span>
                   </div>
                 </div>
               ))}
            </div>
